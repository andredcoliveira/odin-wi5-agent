...
ip addr add $CPAGENTIP/16 dev $CPINTERFACE
ip addr add $DPAGENTIP/16 dev $DPINTERFACE
sleep 1
...
# sleep 1
ip addr add $CPAGENTIP/16 dev $SW
sleep 1
ip addr add $DPAGENTIP/16 dev $SW
sleep 1
...
#cd /mnt/usb
./click < click-align agent.cli &    # This makes the alignment and calls Click at the same time
